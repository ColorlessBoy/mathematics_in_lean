...
theorem neg_eq_of_add_eq_zero {a b : R} (h : a + b = 0) : -a = b := by
  rw [← zero_add b, ← add_left_neg a, add_assoc, h, add_zero]
...
theorem neg_neg (a : R) : - -a = a := by
  rw [← add_zero (- -a), ← add_left_neg a, ← add_assoc, add_left_neg (-a), zero_add]
...
theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  ...
  nth_rw 1 [← one_mul a⁻¹]
  rw [← mul_right_inv b, ← mul_assoc, mul_assoc b, mul_assoc, ← mul_assoc a, ← mul_assoc, mul_left_inv, one_mul]
end MyGroup
